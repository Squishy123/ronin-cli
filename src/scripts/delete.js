...
module.exports = async args => {
    ...
    if (args[0] == 'delete') {
        ...
        let data = {
            models: db.get('models').value().map((m, i) => Object.assign(m, {
                value: {
                    index: i,
                    category: 'models'
                }
            })),
            migrations: db.get('migrations').get('order').value().map((m, i) => Object.assign(m, {
                value: {
                    index: i,
                    category: 'migrations'
                }
            })),
            modules: db.get('modules').value().map((m, i) => Object.assign(m, {
                value: {
                    index: i,
                    category: 'modules'
                }
            })),
            middlewares: db.get('middlewares').value().map((m, i) => Object.assign(m, {
                value: {
                    index: i,
                    category: 'middlewares'
                }
            })),
            routes: db.get('routes').value().map((m, i) => Object.assign(m, {
                value: {
                    index: i,
                    category: 'routes'
                }
        ...
        if(confirm) {
            ...
            ['models', 'modules', 'middlewares', 'routes'].forEach((q) => {
                db.get(q).value().forEach((e, i) => {
                    db.unset(`${q}.${i}.value`).write()
                });
            });

            db.get('migrations').get('order').value().forEach((e, i) => {
                db.unset(`migrations.${i}.value`).write()
            });
        }

        console.log(chalk.greenBright("Deletions") + " Completed Successfully!");
    }
